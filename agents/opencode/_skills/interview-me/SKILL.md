...
description: >
  ...
  Use when an ask is underspecified ("build me X" without "for whom" or "why now"),
...
What people ask for and what they actually want to achieve are different things.
They ask for _"a dashboard"_ because that's what one asks for, not because a dashboard solves their problem.
They say _"make it faster"_ without a number to hit.

The cheapest moment to find this gap is before any plan, spec, or code exists.
Once you've started building, switching costs are real,
and the user will rationalize the wrong thing into a _"good enough"_ thing.
The misfit gets locked in.

This skill closes the gap before it costs anything.
The other Define-phase skills assume you already know roughly what you want:

- [`idea-refine`](../idea-refine/SKILL.md) generates variations from an idea
- [`spec-driven-development`](../spec-driven-development/SKILL.md) writes the requirements down
- [`doubt-driven-development`](../doubt-driven-development/SKILL.md) stress-tests a plan after you've drafted one

`interview-me` skill is the part before all of those, where you ask one question at a time,
with your best guess attached, until you can predict what the user is going to say before they say it.
