...
" Open help with `H on specific plugin,
" when opening with `:PlugStatus` and the cursor is on specific plugin name
function! s:plug_doc()
...
let g:plugins = {
	\ 'dirPath': g:vim_config_dirPath . '/plugins',
	\ 'installed': [],
...
function! g:plugins.set_configurationFilePath(pluginName)
	return fnameescape(l:self.dirPath . '/' . a:pluginName . '.vim')
endfunction

" Load plugin's configuration file
function! g:plugins.load_configurationFile(pluginName)
	let s:config_filePath = l:self.set_configurationFilePath(a:pluginName)
...
call plug#begin(fnameescape(g:vim_home_dirPath . '.vim/plugged'))
	...
	" ----------------------------------------------------------------------- "
		...
		" A fast Neovim statusline plugin written in pure lua
		" -------------------------------------------
		" https://github.com/hoob3rt/lualine.nvim
		Plug 'hoob3rt/lualine.nvim', { 'as': 'lualine' }
		call g:plugins.add_postponed('lualine')
